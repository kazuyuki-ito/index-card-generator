...
<div class="writing-modal">
    <div class="content" :class="{'confirm': confirm}">
        ...
        <div class="modal-menu" v-if="!confirm">
            ...
            <div class="mode" v-if="guide != null">
                <p>ガイド</p>
                <ul>
                    <li><button class="btn white" :class="{'on': guideMode == false}" @click="guidePhoto(false)">なし</button></li>
                    <li><button class="btn white" :class="{'on': guideMode == true}" @click="guidePhoto(true)">あり</button></li>
                </ul>
            </div>
            <div class="hand">
              <p>背景</p>
...
export default {
    props: ['image', 'editCanvas', 'vertical', 'confirm', 'canvasId', 'guide', 'proof'],
    data() {
        return {
            baseCanvas: null,
            canvasData: null,
    ...
    mounted(){
        ...
        this.confirm ? cv.isDrawingMode = false : cv.isDrawingMode = true
        cv.freeDrawingBrush.color = 'red'
        cv.freeDrawingBrush.width = 5
        let history = []
        if(this.guide == 'photo') {
          this.vertical ? this.canvasGuide = '/img/photo_guide_h.png' : this.canvasGuide = '/img/photo_guide_w.png'
        } else if(this.guide == 'print')  {
          this.vertical ? this.canvasGuide = '/img/print_guide_h.png' : this.canvasGuide = '/img/print_guide_w.png'
        } else {
          this.canvasGuide = null
        }
        let vertical = this.vertical
        if(this.image != null) {
          fabric.Image.fromURL(this.image, function(oImg) {
            vertical ? oImg.scaleToHeight(700) : oImg.scaleToWidth(700)

		        oImg.set({
              id: 'temp-image',
              top:  (cv.height - oImg.height * oImg.scaleY) / 2,
			        left: (cv.width - oImg.width * oImg.scaleX) / 2,
              borderColor: '#B2CCFF',
              cornerColor: '#B2CCFF',
          ...
          cv.loadFromJSON(this.editCanvas, function() {
    ...
    methods: {
        ...
        guidePhoto: function(flg) {
          ...
          if(flg) {
            ...
            fabric.Image.fromURL(this.canvasGuide, function(gImg) {

              vertical ? gImg.scaleToHeight(700) : gImg.scaleToWidth(700)

		          gImg.set({
                top:  (cv.height - gImg.height * gImg.scaleY) / 2,
			          left: (cv.width - gImg.width * gImg.scaleX) / 2,
                selectable: false,
		          })
              cv.add(gImg)
            })
